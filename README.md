...
| **L28** | Mutating `src/oracle`                | **Three of the reference's four decision paths were graded by nothing.** The differential skipped every non-admit decision, on an argued-for and plausible comment. Comparing all four: 397 divergences in 15,502 — and the naive fix reversed the sign, because SEMANTICS C5 makes expiry effective on RECLAMATION, not on the clock. |
| **L29** | Mutating `src/core`                  | **Two exported helpers that nothing called** — one of them `byNumberThen`, whose own comment says it exists to remove the ordering ambiguity the determinism claim rests on. A function nothing calls cannot be observed to be wrong, so only mutation could see it.                                                                   |

Full write-ups: [`LEDGER.md`](docs/LEDGER.md).

Seventeen of the twenty-nine were in the **checker, the reference oracle or the harness**, not the
system under test — L1, L11 and L24 (the checker), L3, L12, L23 and L28 (the reference oracle), L7,
L9, L15, L16, L17, L18, L19, L20, L26 and L27 (the guards, the fault injector and the mutation
...
- **300 differential histories**, compared on **every decision the two engines
  make** — admit, release, complete and cancel — and on the rejection _reason_,
  not only the admit/reject bit. It used to skip every non-admit decision, which
  left three of the reference's four paths graded by nothing (L28). Because
  comparing the reason is still not the same as agreeing on which reason WINS,
  there are also **9 hand-built overlap scenarios** in which two or more refusal
  conditions hold at once, each asserting it reaches its overlap before
  asserting the order (L23)
